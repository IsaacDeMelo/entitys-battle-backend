...
// Importa os módulos locais com segurança
let BasePokemon, User, EntityType, MoveType, TypeChart, MOVES_LIBRARY, getXpForNextLevel;
try {
    const models = require('./models');
    BasePokemon = models.BasePokemon;
    User = models.User;
    const gData = require('./gameData');
    EntityType = gData.EntityType; MoveType = gData.MoveType; TypeChart = gData.TypeChart; 
    MOVES_LIBRARY = gData.MOVES_LIBRARY; getXpForNextLevel = gData.getXpForNextLevel;
} catch (e) { console.error("Erro importando módulos:", e); }
...
// --- ESTADOS EM MEMÓRIA (AQUI ESTAVA O PROBLEMA) ---
// Estas variáveis precisam estar definidas no topo!
const activeBattles = {}; 
const onlineBattles = {}; 
const players = {}; 
let matchmakingQueue = []; // <--- AQUI ESTÁ A VARIÁVEL QUE FALTAVA
...
app.get('/', async (req, res) => { 
    ...
    res.render('login', { error: null, skinCount: SKIN_COUNT, starters: starters || [] }); 
});
...
app.post('/login', async (req, res) => {
    try {
        ...
        if (user) {
            ...
            let starters = [];
            try { starters = await BasePokemon.find().limit(3).lean(); } catch(e){}
            res.render('login', { error: 'Credenciais inválidas', skinCount: SKIN_COUNT, starters: starters || [] });
        }
...
app.post('/register', async (req, res) => {
    ...
    try {
        ...
        let starters = [];
        try { starters = await BasePokemon.find().limit(3).lean(); } catch(e){}
        res.render('login', { error: 'Usuário já existe.', skinCount: SKIN_COUNT, starters: starters || [] });
    }
});
...
function processAction(attacker, defender, move, logArray) {
    ...
    logArray.push({ type: 'USE_MOVE', actorId: attacker.instanceId || 'wild', moveName: move.name, moveIcon: move.icon, cost: move.cost });
    ...
    else { let multiplier = 1; if (TypeChart[move.element] && TypeChart[move.element][defender.type] !== undefined) multiplier = TypeChart[move.element][defender.type]; let damage = Math.floor(((move.power + attacker.stats.attack) - defender.stats.defense) * multiplier); if (damage < 1) damage = 1; defender.hp -= damage; logArray.push({ type: 'ATTACK_HIT', attackerId: attacker.instanceId || 'wild', targetId: defender.instanceId || 'wild', damage, newHp: defender.hp, isEffective: multiplier > 1, isNotEffective: multiplier < 1 }); }
}
...
io.on('connection', (socket) => {
    ...
    socket.on('disconnect', () => { 
        ...
        // Correção do erro matchmakingQueue undefined:
        if (matchmakingQueue) matchmakingQueue = matchmakingQueue.filter(u => u.socket.id !== socket.id);
        for(const r in roomSpectators) { if(roomSpectators[r][socket.id]) { delete roomSpectators[r][socket.id]; io.to(r).emit('spectator_left', socket.id); } }
    });
    // Matchmaking PVP
