...
// --- ROTAS (Login, Register, Lobby, Lab, Forest, PC API, Shop API) MANTIDAS IGUAIS ---
...
// POKEDEX: retorna os IDs/baseIds dos pokémons que o usuário já capturou
app.get('/api/pokedex', async (req, res) => {
    ...
    try {
        ...
        const seen = new Set();
        const addFromList = (list) => { if (!list) return; for (const p of list) { if (p && p.baseId) seen.add(String(p.baseId).toLowerCase()); } };
        addFromList(user.pokemonTeam);
        addFromList(user.pc);
        // retornar array de ids
        return res.json({ list: Array.from(seen) });
    } catch (e) {
        console.error('api/pokedex error', e);
...
// BATTLES
app.post('/battle/wild', async (req, res) => { const { userId } = req.body; const user = await User.findById(userId); const userPokeData = user.pokemonTeam.find(p => p.currentHp > 0) || user.pokemonTeam[0]; if(!userPokeData || userPokeData.currentHp <= 0) return res.json({ error: "Todos os seus Pokémons estão desmaiados! Cure-os no Centro." }); const possibleSpawns = await BasePokemon.find({ spawnLocation: 'forest' }); if(possibleSpawns.length === 0) return res.json({ error: "Nenhum pokemon." }); const wildBase = possibleSpawns[Math.floor(Math.random() * possibleSpawns.length)]; const wildLevel = Math.floor(Math.random() * (wildBase.maxSpawnLevel - wildBase.minSpawnLevel + 1)) + wildBase.minSpawnLevel; const wildEntity = await createBattleInstance(wildBase.id, wildLevel); const userBase = await BasePokemon.findOne({ id: userPokeData.baseId }); const userEntity = userPokemonToEntity(userPokeData, userBase); userEntity.playerName = user.username; userEntity.skin = user.skin; const battleId = `wild_${Date.now()}`; activeBattles[battleId] = { p1: userEntity, p2: wildEntity, type: 'wild', userId: user._id, turn: 1 }; res.json({ battleId }); });
...
// --- LÓGICA DE TURNOS E AÇÕES ---
app.post('/api/turn', async (req, res) => {
    // CORREÇÃO: Recebe 'isForced' para saber se foi troca por desmaio
    ...
    try {
        ...
        if (action === 'switch') {
            ...
            // Se for troca voluntária (tática), salva o HP atual
            // Se for forçada (morte), o HP já é 0, não precisa salvar
            if (!isForced) {
                const prevPoke = user.pokemonTeam.find(p => p._id.toString() === p1.instanceId);
                if(prevPoke) { prevPoke.currentHp = p1.hp; } 
            }
            ...
            newEntity.playerName = p1.playerName;
            newEntity.skin = p1.skin;
            ...
            // CORREÇÃO: Oponente SÓ ataca se NÃO for troca forçada (isForced false)
            if (p2.hp > 0 && !isForced) {
            ...
            return res.json({ 
                events, 
                p1State: { hp: p1.hp, energy: p1.energy, maxHp: p1.maxHp, name: p1.name, level: p1.level, sprite: p1.sprite, moves: p1.moves }, 
                p2State: { hp: p2.hp },
                switched: true,
                ...
                try {
                    ...
                    if (Math.random() < chance) {
                        ...
                        if (user.pokemonTeam.length < 6) {
                            user.pokemonTeam.push(newPokeObj);
                            events.push({ type: 'MSG', text: `Gotcha! ${p2.name} capturado!` });
                        } else {
                            user.pc.push(newPokeObj);
                            sentToPC = true;
                            events.push({ type: 'MSG', text: `Time cheio! ${p2.name} foi enviado para o PC.` });
                        }
        ...
        if (p2.hp <= 0) {
            ...
            if (battle.type === 'local') {
                try { const user = await User.findById(battle.userId); if (user) { const reward = Math.max(5, (p2.level || 1) * 5); user.money = (user.money || 0) + reward; await user.save(); events.push({ type: 'MSG', text: `Ganhou ${reward} moedas!` }); } } catch (e) {}
            }
