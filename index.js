...
async function seedDatabase() {
    try {
        ...
        if (count === 0) {
            console.log("🌱 Database empty. Seeding Kanto Starters...");
            const starters = [
...
io.on('connection', (socket) => {
    ...
    socket.on('check_encounter', (data) => { if (data.x < 20 && Math.random() < 0.5) socket.emit('encounter_found'); });
    ...
    socket.on('online_move', ({ roomId, moveId, playerId }) => { 
        ...
        if (battle.p1.ready && battle.p2.ready) { 
            ...
            let winnerId = null; 
            if (p1.hp <= 0 || p2.hp <= 0) { if (p1.hp > 0) winnerId = p1.id; else if (p2.hp > 0) winnerId = p2.id; else winnerId = 'draw'; delete onlineBattles[roomId]; } 
