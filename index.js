...
// IMPORTS LOCAIS (Assumindo que você tem esses arquivos)
...
// --- LÓGICA DE DANO DE STATUS (FINAL DO TURNO) ---
...
function processAction(attacker, defender, move, logArray) {
    ...
    if(move.type === 'heal') { 
        ...
        // Se o golpe for tipo 'poison', alvo não tem status, alvo vivo, e cair nos 25%
...
// Rota de Criação de Pokémon (Admin)
...
app.post('/api/use-item', async (req, res) => { 
    ...
    if(itemId === 'rareCandy') { 
        ...
        let poke = null; 
        try { poke = user.pokemonTeam.id(pokemonId); } 
        catch(e) { poke = user.pokemonTeam.find(p => p._id.toString() === (pokemonId || '')); } 
        ...
        let base = await BasePokemon.findOne({ id: poke.baseId }); 
        let evolved = false;
        ...
        await user.save(); 
        return res.json({ success: true, rareCandy: user.rareCandy, evolved: evolved, pokemon: { instanceId: poke._id, level: poke.level, hp: poke.currentHp, name: poke.nickname } }); 
    } 
...
app.post('/battle', async (req, res) => { 
    ...
    p1.playerName = playerName; p1.skin = playerSkin; 
    ...
    const cpuLevel = Math.max(1, p1.level + (Math.random() > 0.5 ? 1 : -1)); 
    ...
    const p2 = { 
        ...
        level: cpuLevel, 
        hp: s2.hp, maxHp: s2.hp, 
        energy: s2.energy, maxEnergy: s2.energy, 
        stats: s2, 
        moves: cpuMoves.map(mid => ({...MOVES_LIBRARY[mid], id:mid})), 
        sprite: randomBase.sprite, 
        playerName: 'CPU', skin: 'char2',
        status: null 
    }; 
    
    const battleId = 'local_' + Date.now(); 
    activeBattles[battleId] = { p1, p2, type: 'local', userId, turn: 1, mode: 'manual' }; 
...
// --- ROTA CENTRAL DE PROCESSAMENTO DO TURNO ---
app.post('/api/turn', async (req, res) => {
    ...
    try {
        ...
        if (action === 'switch') {
            ...
            newEntity.playerName = p1.playerName; newEntity.skin = p1.skin;
            ...
            // Se foi troca normal (não forçada por morte), inimigo ataca
            if (p2.hp > 0 && !isForced) { 
                performEnemyTurn(p2, p1, events); 
                applyStatusDamage(p1, events); // Dano de status no fim do turno
                applyStatusDamage(p2, events);
            }
            
            return res.json({ 
                events, 
                p1State: { hp: p1.hp, maxHp: p1.maxHp, energy: p1.energy, maxEnergy: p1.maxEnergy, name: p1.name, level: p1.level, sprite: p1.sprite, moves: p1.moves }, 
                p2State: { hp: p2.hp },
                switched: true,
        ...
        if (action === 'catch') {
            if (battle.type !== 'wild') { events.push({ type: 'MSG', text: 'Não é possível capturar aqui.' }); return res.json({ events }); } 
                ...
                try {
                    ...
                    const statusBonus = p2.status ? 0.2 : 0; // Mais fácil capturar se tiver status
