...
const { MONGO_URI } = require('./config'); // Se não tiver config.js, substitua pela string de conexão aqui
...
// Escolhe Pokémon com base na raridade (Spawn Chance)
...
// Calcula atributos com base no nível
...
// Cria instância de batalha (Selvagem)
async function createBattleInstance(baseId, level) { 
    ...
    return { 
        ...
        isWild: true,
        status: null // Status Condition (veneno, etc)
    }; 
}

// Converte Pokémon do Usuário para Entidade de Batalha
...
app.post('/battle/online', (req, res) => {
    ...
    res.render('battle', { p1: me, p2: op, battleMode: 'online', battleId: roomId, myRoleId: me.id, realUserId: me.userId, playerName: me.playerName, playerSkin: me.skin, isSpectator: false, bgImage: 'battle_bg.png', battleData: JSON.stringify({ log: [{type: 'INIT'}] }), switchable: [] });
});

// --- ROTA DE BATALHA TREINO LOCAL ---
...
// --- ROTA GENÉRICA DE BATALHA (VISUALIZAÇÃO) ---
...
// --- ROTA DE PROCESSAMENTO DO TURNO (API CENTRAL) ---
app.post('/api/turn', async (req, res) => {
    ...
    try {
        ...
        if (action === 'switch') {
            ...
            // Salva HP do anterior se não foi forçado
            ...
            // Se troca normal, inimigo ataca e status rolam
            if (p2.hp > 0 && !isForced) { 
        ...
        if (action === 'catch') {
            if (battle.type !== 'wild') { events.push({ type: 'MSG', text: 'Não é possível capturar aqui.' }); return res.json({ events }); } 
                ...
                try {
                    ...
                    const hpPercent = p2.hp / p2.maxHp; const statusBonus = p2.status ? 0.2 : 0;
                    const chance = (p2.catchRate * (1 - hpPercent)) + 0.15 + statusBonus; 
                    if (Math.random() < chance) {
                        ...
                        performEnemyTurn(p2, p1, events); 
                        applyStatusDamage(p1, events); applyStatusDamage(p2, events); 
                    }
                } catch (e) { events.push({ type: 'MSG', text: 'Erro interno.' }); return res.json({ events }); }
            }
        } 
        ...
        // FUGIR (RUN)
            ...
            if (Math.random() > 0.4) { delete activeBattles[battleId]; return res.json({ events: [{type:'MSG', text:'Você fugiu!'}], finished: true, fled: true }); }
                ...
                events.push({ type: 'MSG', text: `Falha ao fugir!` }); 
                performEnemyTurn(p2, p1, events); 
                applyStatusDamage(p1, events); applyStatusDamage(p2, events); 
            }
        } 
        ...
        // ATAQUE (MOVE)
            ...
            // Dano de status no final do turno
            if (p1.hp > 0) applyStatusDamage(p1, events); 
            if (p2.hp > 0) applyStatusDamage(p2, events);
        }
...
// --- SOCKET.IO (CHAT, MAPA, ONLINE BATTLE) ---
io.on('connection', (socket) => {
    ...
    // ONLINE MOVE (PROCESSAMENTO DE PVP)
    socket.on('online_move', async ({ roomId, moveId, playerId }) => { 
        ...
        if (battle.p1.ready && battle.p2.ready) { 
            ...
            if (p1.hp > 0 && p2.hp > 0) { 
                ...
                const runMove = (atk, def) => { if (atk.nextMove === 'rest') { atk.energy += 5; events.push({ type: 'REST', actorId: atk.id, newEnergy: atk.energy }); } else { processAction(atk, def, atk.nextMove, events); } }; 
                
                // Primeiro ataque
                runMove(first, second); 
                applyStatusDamage(first, events);
                
                // Segundo ataque (se vivo)
                if (second.hp > 0) {
                    runMove(second, first);
                    applyStatusDamage(second, events);
                }
            } 
            ...
            if (p1.hp <= 0 || p2.hp <= 0) { 
                ...
                if (betAmount > 0 && winnerId && winnerId !== 'draw') { 
                    try { 
                        ...
                        if (winner && loser) { const actualDeduct = Math.min(loser.money || 0, betAmount); loser.money = Math.max(0, (loser.money || 0) - actualDeduct); winner.money = (winner.money || 0) + actualDeduct; await loser.save(); await winner.save(); events.push({ type: 'MSG', text: `${winner.username} ganhou ${actualDeduct} moedas!` }); } 
