    ...
    try { 
        ...
        if (starterId) { 
            ...
            if (starter) { 
                ...
                if(m.length === 0) m = ['tackle']; 
                starterTeam.push({ baseId: starter.id, nickname: starter.name, level: 1, currentHp: initialStats.hp, stats: initialStats, moves: m, learnedMoves: m }); 
                dex.push(starter.id);
            } 
        } 
...
app.get('/city', async (req, res) => {
    ...
    // Pega o ID do mapa (se vier com lixo na url tipo ?map=house1?x=..., limpa)
    let mapId = map || 'city';
...
app.post('/api/map/save', async (req, res) => {
    ...
    try {
        await GameMap.findOneAndUpdate({ mapId: mapId }, { $set: { collisions: mapData.collisions, grass: mapData.grass, interacts: mapData.interacts, portals: mapData.portals, bgImage: mapData.bgImage, width: mapData.width || 100, height: mapData.height || 100, spawnPoint: mapData.spawnPoint, darknessLevel: mapData.darknessLevel || 0, battleBackground: mapData.battleBackground } }, { upsert: true, new: true });
...
app.get('/api/map/:mapId', async (req, res) => {
    try {
        ...
        if (!map) return res.json({ bgImage: '/uploads/room_bg.png', width: 100, height: 100 });
        res.json(map);
...
app.post('/battle/wild', async (req, res) => { 
    ...
    // --- CORREÇÃO DO LINK DE RETORNO ---
    let returnMapUrl = currentMap;
    // Se não for 'city' nem 'forest' (ou seja, custom map), força query param
...
app.post('/battle/npc', async (req, res) => {
    ...
    let mapName = 'city';
    if (currentMap && currentMap.includes('map=')) {
         const match = currentMap.match(/map=([^&]+)/);
         if (match) mapName = match[1];
    }
    const mapDoc = await GameMap.findOne({ mapId: mapName }).lean();
    ...
    for (let member of npc.team) {
        ...
        if (base) {
            ...
            npcTeamInstances.push({
                instanceId: 'npc_mon_' + Date.now() + Math.random(), 
...
app.get('/battle/:id', async (req, res) => { 
    ...
    res.render('battle', { 
        ...
        battleId: req.params.id, battleMode: battle.type === 'local' ? 'manual' : battle.type, 
        isSpectator: false, myRoleId: battle.p1.instanceId, realUserId: battle.userId, 
        playerName: battle.p1.playerName, playerSkin: battle.p1.skin, 
