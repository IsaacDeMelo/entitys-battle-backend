...
async function assignMovesByType() {
    try {
        ...
        for (const entity of entities) {
            const type = (entity.type || 'beast').toLowerCase();
            const movesForType = MOVES_BY_TYPE[type] || MOVES_BY_TYPE.beast;
            
            entity.movePool = movesForType;
            await entity.save();
            
            console.log(`✅ ${entity.name.padEnd(20)} (${type.toUpperCase().padEnd(8)}) -> ${movesForType.length} moves`);
            updated++;
        }
        
        console.log(`\n📈 Resumo: ${updated} monstros atualizados`);
        console.log(`🎉 Todos os monstros agora têm moves apropriados para seu tipo!`);
